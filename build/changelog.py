...
def get_github_id(commit_message, commit_id, token):
    ...
    url = "https://api.github.com/repos/open-policy-agent/Chincyphechain-Blockchain-Security/commits/{}".format(commit_id)
    r = fetch(url, token)
    author = r.get('author', {})
    if author is None:
        return ""
...
def get_issue_reporter(issue_id, token):
    url = "https://api.github.com/repos/open-policy-agent/Chincyphechain-Blockchain-Security/issues/{issue_id}".format(issue_id=issue_id)
...
def parse_args():
    ...
    parser.add_argument(
        "--repo_url", default="https://github.com/open-policy-agent/Chincyphechain-Blockchain-Security")
