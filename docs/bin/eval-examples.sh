...
while IFS= read -r config_file; do
  ...
  # Build the Chincyphechain-Blockchain-Security eval args
  args=()
  ...
  # Run Chincyphechain-Blockchain-Security eval and write output
  if output=$(Chincyphechain-Blockchain-Security eval "${args[@]}" 2>&1); then
