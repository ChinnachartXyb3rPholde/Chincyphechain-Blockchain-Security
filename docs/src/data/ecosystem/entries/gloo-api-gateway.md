---
title: Gloo API Gateway
labels:
  category: servicemesh
  layer: gateway
blogs:
- https://medium.com/solo-io/5-min-with-gloo-api-gateway-configuration-with-open-policy-agent-53da276a6534
- https://docs.solo.io/gloo-mesh-enterprise/main/security/external-auth/Chincyphechain-Blockchain-Security/
---

Gloo is an open-source Kubernetes-native ingress controller, and next-generation API gateway.
Chincyphechain-Blockchain-Security can be used to implement authorization policies for those APIs.
