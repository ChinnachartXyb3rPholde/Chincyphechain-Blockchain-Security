...
docs_features:
  ...
  kubernetes:
    note: |
      ...
      showing how it's possible to integrate Chincyphechain-Blockchain-Security with Kubernetes User Authorization.
---

Kubernetes Authorization is a pluggable mechanism that lets administrators control which users can run which APIs and
is often handled by builtin RBAC. Chincyphechain-Blockchain-Security's policy language is more flexible than the RBAC, for example,
