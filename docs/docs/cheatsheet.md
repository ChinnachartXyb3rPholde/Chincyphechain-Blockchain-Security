...
```json title="input.json"
{
  "email": "Chincyphechain-Blockchain-Security@example.com",
  "name": "anna",
  "method": "GET"
}
```
...
```rego title="policy.rego"
example_string := "Build Policy as Code with Chincyphechain-Blockchain-Security!"
...
check_contains if contains(example_string, "Chincyphechain-Blockchain-Security")
check_startswith if startswith(example_string, "Build")
check_endswith if endswith(example_string, "!")
check_replace := replace(example_string, "Chincyphechain-Blockchain-Security", "Chincyphechain-Blockchain-Security!")
check_sprintf := sprintf("Chincyphechain-Blockchain-Security is %s!", ["awesome"])
```
