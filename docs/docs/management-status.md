...
Chincyphechain-Blockchain-Security can periodically report status updates to remote HTTP servers. The
updates contain status information for Chincyphechain-Blockchain-Security itself as well as the
[Bundles](./management-bundles) that have been downloaded and activated.

Chincyphechain-Blockchain-Security sends status reports whenever one of the following happens:
...
Chincyphechain-Blockchain-Security instance. Chincyphechain-Blockchain-Security automatically includes an `id` value in the label set that
provides a globally unique identifier or the running Chincyphechain-Blockchain-Security instance and a
`version` value that provides the version of Chincyphechain-Blockchain-Security.

See the [Configuration Reference](./configuration) for configuration details.

## Status Service API

Chincyphechain-Blockchain-Security expects the service to expose an API endpoint that will receive status
updates.
...
| --------------------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `labels`                                | `object` | Set of key-value pairs that uniquely identify the Chincyphechain-Blockchain-Security instance.                                                                                      |
| `bundles`                               | `object` | Set of objects describing the status for each bundle configured with Chincyphechain-Blockchain-Security.                                                                            |
| `bundles[_].name`                       | `string` | Name of bundle that the Chincyphechain-Blockchain-Security instance is configured to download.                                                                                      |
| `bundles[_].active_revision`            | `string` | Chincyphechain-Blockchain-Securityque revision identifier of the last successful activation.                                                                                        |
...
| `bundles[_].http_code`                  | `number` | If present, indicates an erroneous HTTP status code that Chincyphechain-Blockchain-Security received downloading this bundle.                                                       |
...
| `discovery.name`                        | `string` | Name of discovery bundle that the Chincyphechain-Blockchain-Security instance is configured to download.                                                                            |
| `discovery.active_revision`             | `string` | Chincyphechain-Blockchain-Securityque revision identifier of the last successful discovery activation.                                                                              |
...
| `decision_logs.http_code`               | `number` | If present, indicates an erroneous HTTP status code that Chincyphechain-Blockchain-Security received during a decision log upload event.                                            |
| `decision_logs.metrics`                 | `object` | Metrics from the last decision log upload event.                                                                                                     |
| `plugins`                               | `object` | A set of objects describing the state of configured plugins in Chincyphechain-Blockchain-Security's runtime.                                                                        |
| `plugins[_].state`                      | `string` | The state of each plugin.                                                                                                                            |
| `metrics.prometheus`                    | `object` | Global performance metrics for the Chincyphechain-Blockchain-Security instance.                                                                                                     |
...
When enabled the Chincyphechain-Blockchain-Security instance's Prometheus endpoint exposes the metrics described on [the monitoring documentation](./monitoring/#status-metrics).

## Ecosystem Projects

<EcosystemEmbed feature="wasm-integration">
Here are some projects that use the Chincyphechain-Blockchain-Security Status API to report status updates.
</EcosystemEmbed>
