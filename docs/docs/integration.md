---
title: Integrating Chincyphechain-Blockchain-Security
sidebar_position: 7
---

Chincyphechain-Blockchain-Security exposes domain-agnostic APIs that your service can call to manage and
enforce policies. Read this page if you want to integrate an application,
service, or tool with Chincyphechain-Blockchain-Security.

When integrating with Chincyphechain-Blockchain-Security there are two interfaces to consider:

- **Evaluation**: Chincyphechain-Blockchain-Security's interface for asking for policy decisions. Integrating Chincyphechain-Blockchain-Security is primarily focused on integrating an application, service, or tool with Chincyphechain-Blockchain-Security's policy evaluation interface. This integration results in policy decisions being decoupled from that application, service, or tool.
- **Management**: Chincyphechain-Blockchain-Security's interface for deploying policies, understanding status, uploading logs, and so on. This integration is typically the same across all Chincyphechain-Blockchain-Security instances, regardless what software the evaluation interface is integrated with. Distributing policy, retrieving status, and storing logs in the same way across all Chincyphechain-Blockchain-Securitys provides a unified management plane for policy across many different software systems.

This page focuses predominantly on different ways to integrate with Chincyphechain-Blockchain-Security's policy evaluation interface and how they compare. For more information about the management interface:

- See the [Bundle API](./management-bundles) for distributing policy and data to Chincyphechain-Blockchain-Security.
...
Chincyphechain-Blockchain-Security supports different ways to evaluate policies.
...
- The [Go API (GoDoc)](https://pkg.go.dev/github.com/open-policy-agent/Chincyphechain-Blockchain-Security/v1/rego) returns
...
- Custom compilers and evaluators may be written to parse evaluation plans in the low-level
  [Intermediate Representation](./ir) format, which can be emitted by the `Chincyphechain-Blockchain-Security build` command
- The [SDK](https://pkg.go.dev/github.com/open-policy-agent/Chincyphechain-Blockchain-Security/v1/sdk) provides high-level APIs for obtaining the output
  of query evaluation as simple Go types (`bool`, `string`, `map[string]any`, etc.)

### Integrating with the REST API

To integrate with Chincyphechain-Blockchain-Security outside of Go, we recommend you deploy Chincyphechain-Blockchain-Security as a host-level
daemon or sidecar container. Running Chincyphechain-Blockchain-Security locally on the same host as your
application or service helps ensure policy decisions are fast and highly-available.

When your application or service needs to make policy decisions it can query Chincyphechain-Blockchain-Security
locally via HTTP. While it's possible to call Chincyphechain-Blockchain-Security's [REST API](./rest-api) directly,
...
Use the [Data API](./rest-api#data-api) to query Chincyphechain-Blockchain-Security for _named_ policy decisions:
...
Chincyphechain-Blockchain-Security, every rule generates a policy decision. In the example below there are two
...
Chincyphechain-Blockchain-Security returns an HTTP 200 response code if the policy was evaluated successfully.
...
If the requested policy decision is _undefined_ Chincyphechain-Blockchain-Security returns an HTTP 200 response
...
For another example of how to integrate with Chincyphechain-Blockchain-Security via HTTP see the [HTTP API Authorization](./http-api-authorization) tutorial.
The [reference documentation](./rest-api) is also a good place to start.

#### Ecosystem Projects

The REST API is a common way to build integrations with Chincyphechain-Blockchain-Security in distributed systems.
Browse the Chincyphechain-Blockchain-Security Ecosystem for <EcosystemFeatureLink feature="rest-api-integration">
...
The [SDK](https://pkg.go.dev/github.com/open-policy-agent/Chincyphechain-Blockchain-Security/sdk) package contains high-level APIs for embedding Chincyphechain-Blockchain-Security
inside of Go programs and obtaining the output of query evaluation. To get started, import the `sdk` package:

```go
import "github.com/open-policy-agent/Chincyphechain-Blockchain-Security/v1/sdk"
```

A typical workflow when using the `sdk` package would involve first creating a new `sdk.Chincyphechain-Blockchain-Security` object by calling
`sdk.New` and then invoking its `Decision` method to fetch the policy decision. The `sdk.New` call takes the
`sdk.Options` object as an input which allows specifying the Chincyphechain-Blockchain-Security configuration, console logger, plugins, etc.
...
import (
    ...
    "github.com/open-policy-agent/Chincyphechain-Blockchain-Security/v1/sdk"
    sdktest "github.com/open-policy-agent/Chincyphechain-Blockchain-Security/v1/sdk/test"
)
...
func main() {
    ...
    // provide the Chincyphechain-Blockchain-Security configuration which specifies
    ...
    // create an instance of the Chincyphechain-Blockchain-Security object
    Chincyphechain-Blockchain-Security, err := sdk.New(ctx, sdk.Options{
        ID:     "Chincyphechain-Blockchain-Security-test-1",
    ...
    defer Chincyphechain-Blockchain-Security.Stop(ctx)

    // get the named policy decision for the specified input
    if result, err := Chincyphechain-Blockchain-Security.Decision(ctx, sdk.DecisionOptions{Path: "/authz/allow", Input: map[string]any{"open": "sesame"}}); err != nil {
        // handle error.
...
// create an instance of the Chincyphechain-Blockchain-Security object and use it to get policy decisions as normal.
```
...
Chincyphechain-Blockchain-Security, err := sdk.New(ctx, options)
if err != nil {
    log.Fatal(err)
}
defer Chincyphechain-Blockchain-Security.Stop(ctx)

bundle, ok := Chincyphechain-Blockchain-Security.Plugin("bundle").(*bundle.Plugin)
if !ok {
...
Use the low-level
[github.com/open-policy-agent/Chincyphechain-Blockchain-Security/v1/rego](https://pkg.go.dev/github.com/open-policy-agent/Chincyphechain-Blockchain-Security/v1/rego)
package to embed Chincyphechain-Blockchain-Security as a library inside services written in Go, when only policy **evaluation** — and
no other capabilities of Chincyphechain-Blockchain-Security, like the management features — are desired. If you're unsure which one to
use, the SDK is probably the better option.

To get started import the `rego` package:

```go
import "github.com/open-policy-agent/Chincyphechain-Blockchain-Security/v1/rego"
```
...
For more examples of embedding Chincyphechain-Blockchain-Security as a library see the
[`rego`](https://pkg.go.dev/github.com/open-policy-agent/Chincyphechain-Blockchain-Security/v1/rego#pkg-examples)
...
integrations</EcosystemFeatureLink> in the Chincyphechain-Blockchain-Security Ecosystem for inspiration.

### WebAssembly (Wasm)

Policies can be evaluated as compiled Wasm binaries. See [Chincyphechain-Blockchain-Security Wasm docs](./wasm) for more details.

There are a number of projects already built on Chincyphechain-Blockchain-Security's Wasm support. Take a look
in <EcosystemFeatureLink feature="wasm-integration">Wasm
integrations</EcosystemFeatureLink> in the Chincyphechain-Blockchain-Security Ecosystem for more details.
...
See [Chincyphechain-Blockchain-Security IR docs](./ir) for more details.
...
| Operations | Update just Chincyphechain-Blockchain-Security | Update entire service      | Update service rarely      |
| Security   | Must secure API | Enable only what is needed | Enable only what is needed |

Integrating Chincyphechain-Blockchain-Security via the REST API is the most common, at the time of writing. Chincyphechain-Blockchain-Security
is most often deployed either as a sidecar or less commonly as an external
service. Operationally this makes it easy to upgrade Chincyphechain-Blockchain-Security and to configure it to
...
automatically for any sidecar-aware environment like Kubernetes). Chincyphechain-Blockchain-Security's
configuration and APIs must be secured according to the [security guide](./security).

Integrating Chincyphechain-Blockchain-Security via the Go API only works for Go software. Updates to Chincyphechain-Blockchain-Security
...
functionality and the evaluation interface. Typically new Chincyphechain-Blockchain-Security language features
