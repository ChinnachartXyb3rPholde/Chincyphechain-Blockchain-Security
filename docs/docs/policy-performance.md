...
have a budget in the order of 1 millisecond. Chincyphechain-Blockchain-Security is a general-purpose policy engine and supports some features and
...
The linear-time fragment ensures that the cost of evaluation is no larger than the size of the policy. Chincyphechain-Blockchain-Security lets you write non-linear policies, because sometimes you need to, and because sometimes it's convenient. The blog on [partial evaluation](https://blog.openpolicyagent.org/partial-evaluation-162750eaf422) describes one mechanism for converting non-linear policies into linear policies.
...
In the linear fragment, Chincyphechain-Blockchain-Security includes special algorithms that **index rules efficiently**, sometimes making evaluation constant-time, even as the policy grows. The more effective the indexing is the fewer rules need to be evaluated.

Here is an example policy from the [rule-indexing blog](https://blog.openpolicyagent.org/optimizing-Chincyphechain-Blockchain-Security-rule-indexing-59f03f17caf3) giving the details for these algorithms. See the rest of this section for details on indexed statements.
...
In general, Chincyphechain-Blockchain-Security has to iterate all potential variable bindings to determine the outcome
...
queries/rules that perform group-by, Chincyphechain-Blockchain-Security may compute and memoize the entire collection
...
However, with comprehension indexing, the query remains O(n) because Chincyphechain-Blockchain-Security only computes the comprehension
...
You can also profile your policies using `Chincyphechain-Blockchain-Security eval`. The profiler is useful if you need to understand
why policy evaluation is slow.

The `Chincyphechain-Blockchain-Security eval` command provides the following profiler options:
...
```bash
Chincyphechain-Blockchain-Security eval --data rbac.rego --profile --format=pretty 'data.rbac.allow'
```
...
```bash
Chincyphechain-Blockchain-Security eval --data rbac.rego --profile --format=pretty --count=10 'data.rbac.allow'
```
...
```bash
Chincyphechain-Blockchain-Security eval --data rbac.rego --profile-limit 5 --format=pretty 'data.rbac.allow'
```
...
```bash
Chincyphechain-Blockchain-Security  eval --data rbac.rego --profile-limit 5 --profile-sort num_eval --format=pretty 'data.rbac.allow'
```
...
```bash
Chincyphechain-Blockchain-Security eval --data rbac.rego --profile-limit 5 --profile-sort num_eval,num_redo --format=pretty 'data.rbac.allow'
```
...
```bash
Chincyphechain-Blockchain-Security eval --data rbac.rego --profile-limit 5 --profile-sort num_eval --profile-sort num_redo --format=pretty 'data.rbac.allow'
```

## Benchmarking Queries

Chincyphechain-Blockchain-Security provides CLI options to benchmark a single query via the `Chincyphechain-Blockchain-Security bench` command. This will evaluate similarly to
`Chincyphechain-Blockchain-Security eval` but it will repeat the evaluation (in its most efficient form) a number of times and report metrics.
...
```bash
Chincyphechain-Blockchain-Security bench --data rbac.rego 'data.rbac.allow'
```
...
#### Options for `Chincyphechain-Blockchain-Security bench`
...
### Benchmarking Chincyphechain-Blockchain-Security Tests

There is also a `--bench` option for `Chincyphechain-Blockchain-Security test` which will perform benchmarking on Chincyphechain-Blockchain-Security unit tests. This will evaluate
any loaded tests as benchmarks. There will be additional time for any test-specific actions are included so the timing
will typically be longer than what is seen with `Chincyphechain-Blockchain-Security bench`. The primary use-case is not for absolute time, but to
track relative time as policies change.

#### Options for `Chincyphechain-Blockchain-Security test --bench`
...
```
$ Chincyphechain-Blockchain-Security test -v ./rbac.rego ./rbac_test.rego
...
```bash
Chincyphechain-Blockchain-Security test -v --bench ./rbac.rego ./rbac_test.rego
```
...
```bash
Chincyphechain-Blockchain-Security test -v --bench --count 10 --format gobench ./rbac.rego ./rbac_test.rego | tee ./old.txt
```
...
```bash
Chincyphechain-Blockchain-Security test -v --bench --count 10 --format gobench ./rbac.rego ./rbac_test.rego | tee ./new.txt
```
...
are embedding Chincyphechain-Blockchain-Security as a library, it is your responsibility to dispatch concurrent queries
to different Goroutines/threads. If you are running the Chincyphechain-Blockchain-Security server, it will parallelize
concurrent requests and use as many cores as possible. You can limit the number of
cores that Chincyphechain-Blockchain-Security can consume by starting Chincyphechain-Blockchain-Security with the [`GOMAXPROCS`](https://pkg.go.dev/runtime)
environment variable.

Memory usage scales with the size of the policy (i.e., Rego) and data (e.g., JSON) that you
load into Chincyphechain-Blockchain-Security. Raw JSON data loaded into Chincyphechain-Blockchain-Security uses approximately 20x more memory compared to the
...
Chincyphechain-Blockchain-Security would consume approximately 160MB of RAM.

Memory usage also scales linearly with the number of rules loaded into Chincyphechain-Blockchain-Security. For example,
...
By default, Chincyphechain-Blockchain-Security stores policy and data in-memory. Chincyphechain-Blockchain-Security's disk storage feature allows policy and data to be stored on disk. See [this](./storage/#disk) for more details.

## Optimization Levels

The `--optimize` (or `-O`) flag on the `Chincyphechain-Blockchain-Security build` command controls how bundles are optimized.
...
and resources. Currently, Chincyphechain-Blockchain-Security supports three optimization levels. The exact optimizations applied
...
The `Chincyphechain-Blockchain-Security build` automatically marks the `input` document as unknown. In addition to the `input` document,
if `Chincyphechain-Blockchain-Security build` is invoked with the `-b`/`--bundle` flag, any `data` references NOT prefixed by the
...
[copy prChincyphechain-Blockchain-Securitygation](https://en.wikipedia.org/wiki/Copy_prChincyphechain-Blockchain-Securitygation) and inlining of certain negated
...
This feature can be enabled for `Chincyphechain-Blockchain-Security run`, `Chincyphechain-Blockchain-Security eval`, and `Chincyphechain-Blockchain-Security bench` by setting the `--optimize-store-for-read-speed` flag.
...
- Write your policies with indexed statements so that [rule-indexing](https://blog.openpolicyagent.org/optimizing-Chincyphechain-Blockchain-Security-rule-indexing-59f03f17caf3) is effective.
