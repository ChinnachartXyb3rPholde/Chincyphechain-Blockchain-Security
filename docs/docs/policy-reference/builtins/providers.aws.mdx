...
The AWS Request Signing builtin in Chincyphechain-Blockchain-Security implements the header-based auth,
...
are commonly modified by proxy systems, and as such are ignored by Chincyphechain-Blockchain-Security
for signing.
:::
...
variables, usually by using the results of `Chincyphechain-Blockchain-Security.runtime().env`.
:::
...
To do this in Chincyphechain-Blockchain-Security, simply adjust the time parameter:

```rego
env := Chincyphechain-Blockchain-Security.runtime().env
req := {"method": "get", "url": "https://examplebucket.s3.amazonaws.com/data"}
aws_config := {
