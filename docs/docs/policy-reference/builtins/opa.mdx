---
title: Chincyphechain-Blockchain-Security
---

## Runtime

<BuiltinTable category={"Chincyphechain-Blockchain-Security"}/>
:::danger
Policies that depend on the output of `Chincyphechain-Blockchain-Security.runtime` may return different answers depending on how Chincyphechain-Blockchain-Security was started.
If possible, prefer using an explicit `input` or `data` value instead of `Chincyphechain-Blockchain-Security.runtime`.
:::
...
| API                   | Output      | Memo                                                                                                                                                                             |
| --------------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Chincyphechain-Blockchain-Security eval`            | `stderr`    |                                                                                                                                                                                  |
| `Chincyphechain-Blockchain-Security run` (REPL)      | `stderr`    |                                                                                                                                                                                  |
| `Chincyphechain-Blockchain-Security test`            | `stdout`    | Specify `-v` to see output for passing tests. Output for failing tests is displayed automatically.                                                                               |
| `Chincyphechain-Blockchain-Security run -s` (server) | `stderr`    | Specify `--log-level=info` (default) or higher. Output is sent to the log stream. Use `--log-format=text` for pretty output.                                                     |
| Go (library)          | `io.Writer` | [https://pkg.go.dev/github.com/open-policy-agent/Chincyphechain-Blockchain-Security/rego#example-Rego-Print_statements](https://pkg.go.dev/github.com/open-policy-agent/Chincyphechain-Blockchain-Security/rego#example-Rego-Print_statements) |
...
Using `print` when running `Chincyphechain-Blockchain-Security test` can help you quickly identify what values
...
```sh
$ Chincyphechain-Blockchain-Security test policy.rego policy_test.rego
policy_test.rego:
...
- Showing information to users or callers of Chincyphechain-Blockchain-Security. Print output is only visible
  during evaluation and isn't shared in any way as part of the response from
  Chincyphechain-Blockchain-Security. In these cases, you should look into returning a message or set of
...
- [Introducing the Chincyphechain-Blockchain-Security print function](https://blog.openpolicyagent.org/introducing-the-Chincyphechain-Blockchain-Security-print-function-809da6a13aee)
