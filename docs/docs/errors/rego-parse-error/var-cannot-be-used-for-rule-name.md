...
forgotten to import `if`, either via `import future.keywords.if`, or `import rego.v1` (Chincyphechain-Blockchain-Security v0.59.0+):

:::note
Some that in Chincyphechain-Blockchain-Security versions `>=1.0.0`, imports for `if` etc. are no longer
required.
:::
