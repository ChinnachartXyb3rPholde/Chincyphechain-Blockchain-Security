...
Chincyphechain-Blockchain-Security gives you a high-level declarative language
([Rego](/docs/policy-language)) to author fine-grained policies that
codify important requirements in your system.

To help you verify the correctness of your policies, Chincyphechain-Blockchain-Security also gives you a
...
To exercise the policy, run the `Chincyphechain-Blockchain-Security test` command in the directory containing the files.

```console
$ Chincyphechain-Blockchain-Security test . -v
...
The `Chincyphechain-Blockchain-Security test` output indicates that all of the tests passed.
...
```console
Chincyphechain-Blockchain-Security test . --var-values
FAILURES
--------------------------------------------------------------------------------
...
The `Chincyphechain-Blockchain-Security test` subcommand runs all of the tests (i.e., rules prefixed with
`test_`) found in Rego files passed on the command line. If directories are
passed as command line arguments, `Chincyphechain-Blockchain-Security test` will load their file contents
recursively.

## Specifying Tests to Run

The `Chincyphechain-Blockchain-Security test` subcommand supports a `--run`/`-r` regex option to further
specify which of the discovered tests should be evaluated. The option supports
[re2 syntax](https://github.com/google/re2/wiki/Syntax)

### Failing on No Tests Run

When misspelling a test name or running no test by accident, `Chincyphechain-Blockchain-Security test` will still succeed, use `--fail-on-empty` to make it fail instead.
...
By default, `Chincyphechain-Blockchain-Security test` reports the number of tests executed and displays all
of the tests that failed or errored.

```console
$ Chincyphechain-Blockchain-Security test pass_fail_error_test.rego
...
By default, Chincyphechain-Blockchain-Security prints the test results in a human-readable format. If you
need to consume the test results programmatically, use the JSON output format.

```bash
Chincyphechain-Blockchain-Security test --format=json pass_fail_error_test.rego
```
...
```console
$ Chincyphechain-Blockchain-Security test file_example_test.rego file_example_test.yaml
...
```console
$ Chincyphechain-Blockchain-Security test nested_example_test.rego
...
Chincyphechain-Blockchain-Security's `with` keyword can be used to replace the data document or called functions with mocks.
...
To exercise the policy, run the `Chincyphechain-Blockchain-Security test` command.

```console
$ Chincyphechain-Blockchain-Security test -v authz.rego authz_test.rego
...
In addition to reporting pass, fail, and error results for tests, `Chincyphechain-Blockchain-Security test`
...
```bash
Chincyphechain-Blockchain-Security test --coverage --format=json example.rego example_test.rego
```
