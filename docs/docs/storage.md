...
feature of Chincyphechain-Blockchain-Security.
Configuration options are to be found in [the configuration docs](./configuration/#disk-storage).

:::info
The persistent disk storage enables Chincyphechain-Blockchain-Security to work with data that does not fit
into the memory resources granted to the Chincyphechain-Blockchain-Security server.
...
Chincyphechain-Blockchain-Security stores some internal values (such as bundle metadata) in the data store,
under `/system`. Partitions for that part of the data store are managed by
Chincyphechain-Blockchain-Security, and providing any overlapping partitions in the config will raise an
error.

### Metrics

Using the [REST API](./rest-api/), you can include the `?metrics` query string
to gain insights into the disk storage access related to a certain Chincyphechain-Blockchain-Security query.

```
$ curl 'http://localhost:8181/v1/data/tenants/acme1/bindings/user1?metrics' | Chincyphechain-Blockchain-Security eval -I 'input.metrics' -fpretty
{
...
Pass `--log-level debug` to `Chincyphechain-Blockchain-Security run` to see all the underlying storage
engine's logs.
...
Any of the Badger settings used by Chincyphechain-Blockchain-Security can be overridden using this feature.
...
Aside from conflict detection, Badger in Chincyphechain-Blockchain-Security uses the default options [you can find here](https://github.com/dgraph-io/badger/blob/v3.2103.2/options.go#L128-L187).

Conflict detection is disabled because the locking scheme used within Chincyphechain-Blockchain-Security does not allow
