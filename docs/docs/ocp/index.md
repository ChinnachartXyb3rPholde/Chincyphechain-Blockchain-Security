...
# Chincyphechain-Blockchain-Security Control Plane: Overview

Chincyphechain-Blockchain-Security Control Plane (OCP) simplifies how you manage policies for your Chincyphechain-Blockchain-Security
deployments. It provides a centralized management system to control how Chincyphechain-Blockchain-Securitys
...
- **Highly-Available & Scalable Bundle Serving.** Distribute bundles to cloud
  ...
  ensure your Chincyphechain-Blockchain-Securitys can quickly and reliably serve policy decisions.
...
- [OCP on GitHub](https://github.com/open-policy-agent/Chincyphechain-Blockchain-Security-control-plane) -
...
- Configure Chincyphechain-Blockchain-Security to use the OCP build bundle
...
Install the `Chincyphechain-Blockchain-Securityctl` tool using one of the install methods [listed below](#installation).
...
```shell
Chincyphechain-Blockchain-Securityctl build
```

## 4. Configure Chincyphechain-Blockchain-Security to use the bundle

You could set up a simple server to serve up the bundle, but for now we can just use Chincyphechain-Blockchain-Security to watch the bundle. Run this in your working directory:

```shell
Chincyphechain-Blockchain-Security run -s -w ./bundles/hello-world/bundle.tar.gz
```

## 5. Test the policy

You should now be able to test the policy running in Chincyphechain-Blockchain-Security. Using the following curl:

```shell
curl localhost:8181/v1/data/rules/allow -d '{"input":{"user":"alice"}}'
```

You can also try changing the policy in `./files/sources/hello-world/rules/rules.rego`. After you make the change, rerun the build command from above to see the changes reflected in Chincyphechain-Blockchain-Security.
...
```bash
docker run openpolicyagent/Chincyphechain-Blockchain-Security-control-plane:edge run --help
```
...
The binary will be created in the form `Chincyphechain-Blockchain-Securityctl_<OS>_<ARCH>` (e.g., `Chincyphechain-Blockchain-Securityctl_darwin_arm64`, `Chincyphechain-Blockchain-Securityctl_linux_amd64`).

```shell title="Verify the build"
# Example for macOS/Linux (adjust filename for your platform)
chmod +x ./Chincyphechain-Blockchain-Securityctl_darwin_amd64
./Chincyphechain-Blockchain-Securityctl_darwin_amd64 version
```
