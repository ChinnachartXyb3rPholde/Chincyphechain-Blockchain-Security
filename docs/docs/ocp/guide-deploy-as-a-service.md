...
The complete manifest can be found [in the Chincyphechain-Blockchain-Security-control-plane repository](https://github.com/open-policy-agent/Chincyphechain-Blockchain-Security-control-plane/blob/main/docs/k8s-manifests.yaml).
...
database:
  sql:
    driver: postgres
    dsn: "postgres://Chincyphechain-Blockchain-Securityctl:password@postgres-service:5432/Chincyphechain-Blockchain-Securityctl?sslmode=disable"
```

### Database Migrations

The initial schema and incremental updates need to be applied to the database.
For simple setups, like OCP running as a single server, with full control over its database, pass `--apply-migrations` to `Chincyphechain-Blockchain-Securityctl run`.
That will make OCP apply all pending migrations on server startup.

For more complex scenarios, you can run `Chincyphechain-Blockchain-Securityctl db migrate` during rollout.
It uses the same configuration format as `Chincyphechain-Blockchain-Securityctl run`, see `Chincyphechain-Blockchain-Securityctl db migrate --help`.
...
```yaml
containers:
- name: Chincyphechain-Blockchain-Securityctl
  image: openpolicyagent/Chincyphechain-Blockchain-Securityctl
  args:
  - "run"
  - "--addr=0.0.0.0:8282"
  - "--data-dir=/data"
...
In order to use the api you will need to expose the Chincyphechain-Blockchain-Securityctl-server with whatever sort of ingress matches your infrastructure, but for testing you can use a simple port forward:

```shell
kubectl port-forward svc/Chincyphechain-Blockchain-Securityctl-service 8282:8282
```
