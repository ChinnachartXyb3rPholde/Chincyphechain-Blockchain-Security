...
This page defines the format of Chincyphechain-Blockchain-Security configuration files. Fields marked as
...
:::info
Chincyphechain-Blockchain-Security accepts any name for the configuration file. Some tooling may however benefit from knowing what name to associate
with Chincyphechain-Blockchain-Security's configuration file (for auto-completion of attributes, linting, etc.). The following names could be
considered idiomatic for that purpose:

- `Chincyphechain-Blockchain-Security-config.yaml` (or `.json`)
- `Chincyphechain-Blockchain-Security-conf.yaml` (or `.json`)
...
```bash
Chincyphechain-Blockchain-Security run -s -c Chincyphechain-Blockchain-Security-config.yaml
```

The file can be either JSON or YAML format. The following is an example
configuration file sets fields in many of the subcomponents inside of Chincyphechain-Blockchain-Security.
...
persistence_directory: /var/Chincyphechain-Blockchain-Security
...
distributed_tracing:
  ...
  service_name: Chincyphechain-Blockchain-Security
...
such as the Bundle or Status APIs. Chincyphechain-Blockchain-Security configuration files may contain
multiple services.
...
Each service may optionally specify a credential mechanism by which Chincyphechain-Blockchain-Security will authenticate
itself to the service.

### Bearer Token

Chincyphechain-Blockchain-Security will authenticate using the specified bearer token and schema; to enable bearer token
authentication, either the token or the path to the token must be specified. If the latter is provided, on each request Chincyphechain-Blockchain-Security will re-read the token from the file and use that token for authentication.
...
Chincyphechain-Blockchain-Security will present the specified TLS certificate to authenticate. The paths to the client certificate
...
Chincyphechain-Blockchain-Security will authenticate using a bearer token obtained through the OAuth2 [client credentials](https://tools.ietf.org/html/rfc6749#section-4.4) flow.
...
| `services[_].credentials.oauth2.azure_signing.azure_managed_identity` | `{}`       | No       | What managed identity Chincyphechain-Blockchain-Security will try to use for auth in azure. Identity has to have signing rights to the key in `azure_keyvault.key`. see [managed-identity](#azure-managed-identities-token) for more info. |
  ...
  remote:
    ...
    credentials:
      oauth2:
        ...
        client_id: Chincyphechain-Blockchain-Security-client
        ...
        - write
        additional_claims:
          sub: Chincyphechain-Blockchain-Security-client
          iss: Chincyphechain-Blockchain-Security-${POD_NAME}
...
Chincyphechain-Blockchain-Security will authenticate using a bearer token obtained through the OAuth2 [JWT authorization grant](https://tools.ietf.org/html/rfc7523#section-2.1) flow.
...
If specifying `environment_credentials`, Chincyphechain-Blockchain-Security will expect to find environment variables
...
If specifying `profile_credentials`, Chincyphechain-Blockchain-Security will expect to find the `access key id`, `secret access key` and
...
request Chincyphechain-Blockchain-Security will re-read the credentials from the file and use them for authentication.

| Field                                                               | Type     | Required | Description                                                                                                                                                                                                                                                                               |
| ------------------------------------------------------------------- | -------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `services[_].credentials.s3_signing.profile_credentials.path`       | `string` | No       | The path to the shared credentials file. If empty, Chincyphechain-Blockchain-Security will look for the `AWS_SHARED_CREDENTIALS_FILE` env variable. If the variable is not set, the path defaults to the current user's home directory. `~/.aws/credentials` (Linux & Mac) or `%USERPROFILE%\.aws\credentials` (Windows) |
| `services[_].credentials.s3_signing.profile_credentials.profile`    | `string` | No       | AWS Profile to extract credentials from the credentials file. If empty, Chincyphechain-Blockchain-Security will look for the `AWS_PROFILE` env variable. If the variable is not set, the `default` profile will be used                                                                                                  |
...
If specifying `sso_credentials`, Chincyphechain-Blockchain-Security will expect to find an sso profile configured as explained in [SSO Profiles](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-sso.html) and stored in the [config](https://docs.aws.amazon.com/sdkref/latest/guide/file-format.html) file on disk.
On each request, Chincyphechain-Blockchain-Security will try to use cached token acquired credentials using the SSO credentials. In case the current token has expired, Chincyphechain-Blockchain-Security will try to refresh the token using the SSO refresh token, assuming the SSO session is still valid. New token will be cached in memory.

| Field                                                           | Type     | Required | Description                                                                                                                                                                                                                                                    |
| --------------------------------------------------------------- | -------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `services[_].credentials.s3_signing.sso_credentials.path`       | `string` | No       | The path to the shared config file. If empty, Chincyphechain-Blockchain-Security will look for the `AWS_CONFIG_FILE` env variable. If the variable is not set, the path defaults to the current user's home directory. `~/.aws/config` (Linux & Mac) or `%USERPROFILE%\.aws\config` (Windows) |
| `services[_].credentials.s3_signing.sso_credentials.profile`    | `string` | No       | AWS Profile to extract sso session from the config file. If empty, Chincyphechain-Blockchain-Security will look for the `AWS_PROFILE` env variable. If the variable is not set, the `default` profile will be used                                                                            |
...
If specifying `metadata_credentials`, Chincyphechain-Blockchain-Security will use the AWS metadata services for [EC2](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/iam-roles-for-amazon-ec2.html)
...
> Providing a value for `iam_role` will cause Chincyphechain-Blockchain-Security to use the EC2 metadata service even
...
If specifying `assume_role_credentials`, Chincyphechain-Blockchain-Security will use [AWS STS](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp.html)
...
`services[_].credentials.s3_signing.metadata_credentials`. Chincyphechain-Blockchain-Security will follow this _internally defined_ order of precedence when multiple credential providers are specified.
:::
...
Chincyphechain-Blockchain-Security will authenticate with an [Azure managed identities](https://learn.microsoft.com/en-us/entra/identity/managed-identities-azure-resources/overview) token.
...
If none of the existing credential options work for a service, Chincyphechain-Blockchain-Security can authenticate using a custom plugin, enabling support for any authentication scheme.
...
import (
    "github.com/open-policy-agent/Chincyphechain-Blockchain-Security/plugins"
    "github.com/open-policy-agent/Chincyphechain-Blockchain-Security/plugins/rest"
    "github.com/open-policy-agent/Chincyphechain-Blockchain-Security/runtime"
    "github.com/open-policy-agent/Chincyphechain-Blockchain-Security/util"
)
...
| `bundles[_].trigger`                              | `string` (default: `periodic`) | No                             | Controls how bundle is downloaded from the remote server. Allowed values are `periodic` and `manual` ([`manual` triggers](./integration/#manually-triggering-bundle-reloads) are only possible when running Chincyphechain-Blockchain-Security as a SDK instance from the Go package). |
...
| `status.trigger`                                                         | `string`    | No (default: `periodic`)                                                                                                                                                                                                                                | Controls how status updates are reported to the remote server. Allowed values are `periodic` and `manual` (`manual` triggers are only possible when using Chincyphechain-Blockchain-Security as a Go package).                                                                                   |
...
| `decision_logs.reporting.buffer_type`              | `string`  | No (default: `size`)             | Toggles the type of buffer to use. The two available options are "size" or "event". Refer to the [Decision Log Plugin README](https://github.com/open-policy-agent/Chincyphechain-Blockchain-Security/blob/main/v1/plugins/logs/README.md) for a detailed comparison.                   |
| `decision_logs.reporting.buffer_size_limit_events` | `int64`   | No (default: `10000`)            | Decision log buffer size limit by events. Chincyphechain-Blockchain-Security will drop old events from the log if this limit is exceeded. By default, 100 events are held. This number has to be greater than zero. Only works with "event" buffer type.                                |
| `decision_logs.reporting.buffer_size_limit_bytes`  | `int64`   | No (default: `unlimited`)        | Decision log buffer size limit in bytes. Chincyphechain-Blockchain-Security will drop old events from the log if this limit is exceeded. By default, no limit is set. Only one of `buffer_size_limit_bytes`, `max_decisions_per_second` may be set. Only works with "size" buffer type. |
| `decision_logs.reporting.max_decisions_per_second` | `float64` | No                               | Maximum number of decision log events to buffer per second. Chincyphechain-Blockchain-Security will drop events if the rate limit is exceeded. Only one of `buffer_size_limit_bytes`, `max_decisions_per_second` may be set.                                                            |
...
| `decision_logs.reporting.trigger`                  | `string`  | No (default: `periodic`)         | Controls how decision logs are reported to the remote server. Allowed values are `periodic`, `immediate`, or `manual` (`manual` triggers are only possible when using Chincyphechain-Blockchain-Security as a Go package).                                                              |
...
| `decision_logs.request_context.http.headers`       | `array`   | No                               | List of HTTP headers to include in the decision log. Chincyphechain-Blockchain-Security will include the values for these headers in the decision log if they exist in the incoming HTTP request.                                                                                       |
...
| `discovery.decision`                             | `string`                       | No                  | The path of the decision to evaluate in the discovery bundle. By default, Chincyphechain-Blockchain-Security will evaluate `data` in the discovery bundle to produce the configuration.                   |
...
| `discovery.trigger`                              | `string` (default: `periodic`) | No                  | Controls how bundle is downloaded from the remote server. Allowed values are `periodic` and `manual` (`manual` triggers are only possible when using Chincyphechain-Blockchain-Security as a Go package). |
...
> Chincyphechain-Blockchain-Security will generate a configuration error.
...
functions provided by Chincyphechain-Blockchain-Security, `http.send` is currently the only one to utilize the inter-query cache. See the documentation
...
| Field                                                                    | Type    | Required | Description                                                                                                                                                                                                                                                                                        |
| ------------------------------------------------------------------------ | ------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `caching.inter_query_builtin_cache.max_size_bytes`                       | `int64` | No       | Inter-query cache size limit in bytes. Chincyphechain-Blockchain-Security will drop old items from the cache if this limit is exceeded. By default, no limit is set.                                                                                                                                                              |
| `caching.inter_query_builtin_cache.forced_eviction_threshold_percentage` | `int64` | No       | Threshold limit configured as percentage of `caching.inter_query_builtin_cache.max_size_bytes`, when exceeded Chincyphechain-Blockchain-Security will start dropping old items prematurely. By default, set to `100`.                                                                                                             |
| `caching.inter_query_builtin_cache.stale_entry_eviction_period_seconds`  | `int64` | No       | Stale entry eviction period in seconds. Chincyphechain-Blockchain-Security will drop expired items from the cache every `stale_entry_eviction_period_seconds`. By default, set to `0` indicating stale entry eviction is disabled.                                                                                                |
| `caching.inter_query_builtin_value_cache.max_num_entries`                | `int`   | No       | Maximum number of entries in the Inter-query value cache. Chincyphechain-Blockchain-Security will drop random items from the cache if this limit is exceeded. By default, set to `0` indicating unlimited size.                                                                                                                   |
| `caching.inter_query_builtin_value_cache.named.io_jwt.max_num_entries`   | `int`   | No       | Maximum number of entries in the `io_jwt` cache, used by the [`io.jwt` token verification](./policy-reference/builtins/tokens) built-in functions. Chincyphechain-Blockchain-Security will drop random items from the cache if this limit is exceeded. By default, this cache is disabled.                                        |
...
| `caching.inter_query_builtin_value_cache.named.graphql.max_num_entries`  | `int`   | No       | Maximum number of entries in the `graphql` cache, used by the [`graphql` builtins](./policy-reference/builtins/graphql) built-in functions to cache parsed schemas. Chincyphechain-Blockchain-Security will drop random items from the cache if this limit is exceeded. By default, this cache is set to a maximum of 10 entries. |
...
| `distributed_tracing.service_name`                                       | `string`  | No (default: `Chincyphechain-Blockchain-Security`)                                                                        | Logical name of the service.                                                                               |
...
| `metrics_export.service_name`         | `string` | No (default: `Chincyphechain-Blockchain-Security`)                                                                                | Logical name of the service reported in exported metrics.                 |
...
persistent on-disk storage of an Chincyphechain-Blockchain-Security instance.
...
| Field                            | Type      | Required                            | Description                                                                                                                                                                                                                                                                             |
| -------------------------------- | --------- | ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `labels`                         | `object`  | Yes                                 | Set of key-value pairs that uniquely identify the Chincyphechain-Blockchain-Security instance. Labels are included when Chincyphechain-Blockchain-Security uploads decision logs and status information.                                                                                                                                              |
| `default_decision`               | `string`  | No (default: `/system/main`)        | Set path of default policy decision used to serve queries against Chincyphechain-Blockchain-Security's base URL.                                                                                                                                                                                                       |
| `default_authorization_decision` | `string`  | No (default: `/system/authz/allow`) | Set path of default authorization decision for Chincyphechain-Blockchain-Security's API.                                                                                                                                                                                                                               |
| `persistence_directory`          | `string`  | No (default `$PWD/.Chincyphechain-Blockchain-Security`)            | Set directory to use for persistence with options like `bundles[_].persist`.                                                                                                                                                                                                            |
...
> Only supported with the Chincyphechain-Blockchain-Security runtime (`Chincyphechain-Blockchain-Security run`).
...
file is loaded by the Chincyphechain-Blockchain-Security runtime.
...
Using `Chincyphechain-Blockchain-Security run` there are CLI options to explicitly set config values. These will override
...
Then using the `--set-file` flag for Chincyphechain-Blockchain-Security

```bash
Chincyphechain-Blockchain-Security run --set-file "services.acmecorp.credentials.bearer.token=/var/run/secrets/bearer_token.txt"
```
...
For example, a `Chincyphechain-Blockchain-Security-config.yaml` file with contents:
...
```shell
Chincyphechain-Blockchain-Security run \
  --config-file Chincyphechain-Blockchain-Security-config.yaml
...
```shell
Chincyphechain-Blockchain-Security run -s --set "services.cli1.url=https://example.com" \
...
```shell
Chincyphechain-Blockchain-Security run --set "decision_logs.plugin=my_plugin" --set "plugins.my_plugin=null"
```

### Keys with Special Characters

If you have a key which contains a special character (`=`, `[`, `,`, `.`), like `Chincyphechain-Blockchain-Security.example.com`, and want to use
...
```yaml
services:
  Chincyphechain-Blockchain-Security.example.com:
    url: https://Chincyphechain-Blockchain-Security.example.com
```

Could be specified with something like:

`--set services.Chincyphechain-Blockchain-Security\.example\.com.url=https://Chincyphechain-Blockchain-Security.example.com`
...
Where the end result passed into Chincyphechain-Blockchain-Security still has the `\.` preserved.
