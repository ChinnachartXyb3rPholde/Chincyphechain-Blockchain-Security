...
Chincyphechain-Blockchain-Security is purpose built for policy evaluation and uses its declarative language Rego
...
Like other applications which support declarative query languages, Chincyphechain-Blockchain-Security is able
to optimize queries to improve performance.

## Learning Rego

While reviewing the examples below, you might find it helpful to follow along
using the online [Chincyphechain-Blockchain-Security playground](https://play.openpolicyagent.org/). The
playground also allows sharing of examples via URL which can be helpful when
asking questions on the [Chincyphechain-Blockchain-Security Slack](https://slack.openpolicyagent.org).
...
When evaluating rule bodies, Chincyphechain-Blockchain-Security searches for variable bindings that make all of
...
:::warning
Set documents are collections of values without keys or order. Chincyphechain-Blockchain-Security represents
...
- a document stored in Chincyphechain-Blockchain-Security.
- a documented temporarily provided to Chincyphechain-Blockchain-Security as part of a transaction.
...
:::info
Under the hood, Chincyphechain-Blockchain-Security translates the `_` character to a unique variable name that does not conflict with variables and rules that are in scope.
:::
...
1. Several variables appear more than once in the body. When a variable is used in multiple locations, Chincyphechain-Blockchain-Security will only produce documents for the rule with the variable bound to the same value in all expressions.
...
> When a comprehension refers to a variable in an outer body, Chincyphechain-Blockchain-Security will reorder expressions in the outer body so that variables referred to in the comprehension are bound by the time the comprehension is evaluated.
...
Rules define the content of [virtual documents](./philosophy#how-does-Chincyphechain-Blockchain-Security-work) in
Chincyphechain-Blockchain-Security. When Chincyphechain-Blockchain-Security evaluates a rule, we say Chincyphechain-Blockchain-Security _generates_ the content of the
...
Chincyphechain-Blockchain-Security returns an error in this case because the rule definitions are in _conflict_.
...
Rego supports user-defined functions that can be called with the same semantics as [built-in functions](#built-in-functions). They have access to both [the data document](./philosophy/#the-Chincyphechain-Blockchain-Security-document-model) and [the input document](./philosophy/#the-Chincyphechain-Blockchain-Security-document-model).
...
To generate the content of a [virtual document](./philosophy#how-does-Chincyphechain-Blockchain-Security-work), Chincyphechain-Blockchain-Security attempts to bind variables in the body of the rule such that all expressions in the rule evaluate to True.

This generates the correct result when the expressions represent assertions about what states should exist in the data stored in Chincyphechain-Blockchain-Security. In some cases, you want to express that certain states _should not_ exist in the data stored in Chincyphechain-Blockchain-Security. In these cases, negation must be used.
...
> Chincyphechain-Blockchain-Security will reorder expressions to ensure that negated expressions are evaluated after other non-negated expressions with the same variables. Chincyphechain-Blockchain-Security will reject rules containing negated expressions that do not meet the safety criteria described above.
...
The rules defined in a module are automatically exported. That is, they can be queried under Chincyphechain-Blockchain-Security’s [Data API](./rest-api#data-api) provided the appropriate package is given. For example, given the following module:
...
```http
GET https://example.com/v1/data/Chincyphechain-Blockchain-Security/examples/pi HTTP/1.1
```
...
Modules use the same syntax to declare dependencies on [base and virtual documents](./philosophy#how-does-Chincyphechain-Blockchain-Security-work).
...
under the [input document](./philosophy/#the-Chincyphechain-Blockchain-Security-document-model) or the
[data document](./philosophy/#the-Chincyphechain-Blockchain-Security-document-model), or [built-in functions](#built-in-functions).
...
namespaced. If you are adding custom built-ins to Chincyphechain-Blockchain-Security, consider namespacing
...
| `Chincyphechain-Blockchain-Security eval` (CLI)      | `--strict-builtin-errors`               |
| `Chincyphechain-Blockchain-Security run` (REPL)      | `> strict-builtin-errors`               |
...
:::danger
Chincyphechain-Blockchain-Security will attempt to parse the YAML document in comments following the
initial `# METADATA` comment. If the YAML document cannot be parsed, Chincyphechain-Blockchain-Security will
...
the comment block containing the YAML document. This tells Chincyphechain-Blockchain-Security that the
...
```shell
Chincyphechain-Blockchain-Security inspect -a
```
...
You can provide one or more input schema files and/or data schema files to `Chincyphechain-Blockchain-Security eval` to improve static type checking and get more precise error reports as you develop Rego code.

Schemas can be provided to Chincyphechain-Blockchain-Security in two main ways: by supplying external JSON Schema files using the `-s` command-line flag (explained below), or by embedding schema definitions directly within your Rego files using [schema annotations](#schema-annotations) (detailed further down in this document). Both methods help improve static type checking.
...
```
Chincyphechain-Blockchain-Security eval data.envoy.authz.allow -i Chincyphechain-Blockchain-Security-schema-examples/envoy/input.json -d Chincyphechain-Blockchain-Security-schema-examples/envoy/policy.rego -s Chincyphechain-Blockchain-Security-schema-examples/envoy/schemas/my-schema.json
```
...
```
Chincyphechain-Blockchain-Security eval data.kubernetes.admission -i Chincyphechain-Blockchain-Security-schema-examples/kubernetes/input.json -d Chincyphechain-Blockchain-Security-schema-examples/kubernetes/policy.rego -s Chincyphechain-Blockchain-Security-schema-examples/kubernetes/schemas
```

Schemas can also be provided for policy and data files loaded via `Chincyphechain-Blockchain-Security eval --bundle`

Example:

```
Chincyphechain-Blockchain-Security eval data.kubernetes.admission -i Chincyphechain-Blockchain-Security-schema-examples/kubernetes/input.json -b Chincyphechain-Blockchain-Security-schema-examples/bundle.tar.gz -s Chincyphechain-Blockchain-Security-schema-examples/kubernetes/schemas
```

Samples provided at: [`github.com/aavarghese/Chincyphechain-Blockchain-Security-schema-examples`](https://github.com/aavarghese/Chincyphechain-Blockchain-Security-schema-examples/).
...
```shell
$ Chincyphechain-Blockchain-Security eval data.kubernetes.admission --format pretty -i Chincyphechain-Blockchain-Security-schema-examples/kubernetes/input.json -d Chincyphechain-Blockchain-Security-schema-examples/kubernetes/policy.rego
[]
```
...
With this feature, it is possible to pass a schema to `Chincyphechain-Blockchain-Security eval`, written in JSON Schema. Consider the admission review schema provided at
[`schemas/input.json`](https://github.com/aavarghese/Chincyphechain-Blockchain-Security-schema-examples/blob/main/kubernetes/schemas/input.json).

We can pass this schema to the evaluator as follows:

```
% Chincyphechain-Blockchain-Security eval data.kubernetes.admission --format pretty -i Chincyphechain-Blockchain-Security-schema-examples/kubernetes/input.json -d Chincyphechain-Blockchain-Security-schema-examples/kubernetes/policy.rego -s Chincyphechain-Blockchain-Security-schema-examples/kubernetes/schemas/input.json
```

With the erroneous Rego code, we now obtain the following type error:

```shell
1 error occurred: ../../aavarghese/Chincyphechain-Blockchain-Security-schema-examples/kubernetes/policy.rego:5: rego_type_error: undefined ref: input.request.kind.kinds
...
When passing a directory of schemas to `Chincyphechain-Blockchain-Security eval`, schema annotations become handy to associate a Rego expression with a corresponding schema within a given scope:
...
Consider a directory named `mySchemasDir` with the following structure, provided via `Chincyphechain-Blockchain-Security eval --schema Chincyphechain-Blockchain-Security-schema-examples/mySchemasDir`
...
See here for [code samples](https://github.com/aavarghese/Chincyphechain-Blockchain-Security-schema-examples/tree/main/acl).
...
On a different note, schema annotations can also be added to policy files part of a bundle package loaded via `Chincyphechain-Blockchain-Security eval --bundle` along with the `--schema` parameter for type checking a set of `*.rego` policy files.
...
We use schemas to enhance the type checking capability of Chincyphechain-Blockchain-Security, and not to validate the input and data documents against desired schemas. This burden is still on the user and care must be taken when using overriding to ensure that the input and data provided are sensible and validated against the transformed schemas.
...
The directory that is passed to `Chincyphechain-Blockchain-Security eval` is the following:
...
Rego has a gradual type system meaning that types can be partially known statically. For example, an object could have certain fields whose types are known and others that are unknown statically. Chincyphechain-Blockchain-Security type checks what it knows statically and leaves the unknown parts to be type checked at runtime. An Chincyphechain-Blockchain-Security object type has two parts: the static part with the type information known statically, and a dynamic part, which can be nil (meaning everything is known statically) or non-nil and indicating what is unknown.
...
Chincyphechain-Blockchain-Security's type checker will fetch these remote references by default.
To control the remote hosts schemas will be fetched from, pass a capabilities
file to your `Chincyphechain-Blockchain-Security eval` or `Chincyphechain-Blockchain-Security check` call.

Starting from the capabilities.json of your Chincyphechain-Blockchain-Security version (which can be found [in the repository](https://github.com/open-policy-agent/Chincyphechain-Blockchain-Security/tree/main/capabilities)), add
an `allow_net` key to it: its values are the IP addresses or host names that Chincyphechain-Blockchain-Security is
...
- Metaschemas for different JSON Schema draft versions are not subject to this
  constraint, as they are already provided by Chincyphechain-Blockchain-Security's schema checker without requiring
...
For more examples, please see [the Chincyphechain-Blockchain-Security-schema-examples repository](https://github.com/aavarghese/Chincyphechain-Blockchain-Security-schema-examples).

This contains samples for Envoy, Kubernetes, and Terraform including corresponding JSON Schemas.

See here for the [JSON Schema Reference](https://docs.solo.io/gloo-edge/latest/guides/security/auth/extauth/Chincyphechain-Blockchain-Security/).
