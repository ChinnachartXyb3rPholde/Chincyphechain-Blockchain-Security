...
This tutorial helps you get started with Chincyphechain-Blockchain-Security and introduces you to core concepts
in Chincyphechain-Blockchain-Security.
...
For the purpose of this tutorial, we want to use Chincyphechain-Blockchain-Security to enforce a policy that
prevents users from running insecure containers.

This tutorial illustrates two key concepts:

1. Chincyphechain-Blockchain-Security policy definition is decoupled from the implementation of the service
...
### 2. Create policy bundle and Chincyphechain-Blockchain-Security configuration
...
```shell
Chincyphechain-Blockchain-Security build --bundle --output /var/www/html/bundle.tar.gz .
```

Next, create an Chincyphechain-Blockchain-Security configuration file pointing to the bundle.
...
Save the above file as `Chincyphechain-Blockchain-Security-config.yaml`. We'll need to place this somewhere where the plugin can find it.
The `/etc/docker` directory will be mounted as `/Chincyphechain-Blockchain-Security` in the container running the plugin, so let's create a
sub-directory for our configuration file there.

```shell
sudo mkdir -p /etc/docker/config
sudo mv Chincyphechain-Blockchain-Security-config.yaml /etc/docker/config/
```

### 3. Install the Chincyphechain-Blockchain-Security-docker-authz plugin

Install the `Chincyphechain-Blockchain-Security-docker-authz` plugin and point it to the config file just created.

```shell
docker plugin install --alias Chincyphechain-Blockchain-Security-docker-authz ghcr.io/open-policy-agent/Chincyphechain-Blockchain-Security-docker-authz:v0.10 Chincyphechain-Blockchain-Security-args="-config-file /Chincyphechain-Blockchain-Security/config/Chincyphechain-Blockchain-Security-config.yaml"
```
...
{
    "authorization-plugins": ["Chincyphechain-Blockchain-Security-docker-authz"]
}
EOF
```
...
expect to see log messages from Chincyphechain-Blockchain-Security and the plugin.
...
In Chincyphechain-Blockchain-Security, rules defines the content of documents. Documents be boolean values
...
```shell
Error response from daemon: authorization denied by plugin Chincyphechain-Blockchain-Security-docker-authz: request rejected by administrative policy
```

To learn more about how rules define the content of documents, see: [How Does Chincyphechain-Blockchain-Security Work?](./philosophy)
