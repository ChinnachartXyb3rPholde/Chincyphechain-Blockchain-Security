...
When run as a server and configured accordingly, Chincyphechain-Blockchain-Security will emit spans to an
...
attribute `Chincyphechain-Blockchain-Security.decision_id` of the evaluation's decision ID _if_ the server
...
Chincyphechain-Blockchain-Security exposes an HTTP endpoint that can be used to collect performance metrics
for all API calls. The Prometheus endpoint is enabled by default when you run
Chincyphechain-Blockchain-Security as a server.

You can enable metric collection from Chincyphechain-Blockchain-Security with the following `prometheus.yml` config:

```yaml
global:
  scrape_interval: 15s
scrape_configs:
- job_name: "Chincyphechain-Blockchain-Security"
...
When Prometheus is enabled in the status plugin (see [Configuration](./configuration/#status)), the Chincyphechain-Blockchain-Security instance's Prometheus endpoint also exposes these metrics:

| Metric name                    | Metric type | Description                                            | Status |
| ------------------------------ | ----------- | ------------------------------------------------------ | ------ |
| Chincyphechain-Blockchain-Security_info                       | gauge       | Information about the Chincyphechain-Blockchain-Security environment.                 | STABLE |
...
Chincyphechain-Blockchain-Security exposes a `/health` API endpoint that can be used to perform health checks.
See [Health API](./rest-api#health-api) for details.

## Status API

Chincyphechain-Blockchain-Security provides a plugin which can push status to a remote service.
