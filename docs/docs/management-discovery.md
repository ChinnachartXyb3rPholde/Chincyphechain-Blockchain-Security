...
Chincyphechain-Blockchain-Security can be configured to download bundles of policy and data, report status, and
upload decision logs to remote endpoints. The discovery feature helps you
centrally manage the Chincyphechain-Blockchain-Security configuration for these features. You should use the
discovery feature if you want to avoid managing Chincyphechain-Blockchain-Security configuration updates in
a number of different locations.

When the discovery feature is enabled, Chincyphechain-Blockchain-Security will periodically download a
_discovery bundle_. Like regular bundles, the discovery bundle may contain JSON
and Rego files. Chincyphechain-Blockchain-Security will evaluate the data and policies contained in the
discovery bundle to generate the rest of the configuration. There are two main
ways to structure the discovery bundle:

1. Include static JSON configuration files that define the Chincyphechain-Blockchain-Security configuration.
2. Include Rego files that can be evaluated to produce the Chincyphechain-Blockchain-Security configuration.

> If you need Chincyphechain-Blockchain-Security to select which policy to download dynamically (e.g., based on
> environment variables like the region where Chincyphechain-Blockchain-Security is running), use the second
> option.
...
Chincyphechain-Blockchain-Security expects the service to expose an API endpoint that serves bundles.
...
You can enable discovery with an Chincyphechain-Blockchain-Security configuration file similar to the example
below. In some places in the documentation, the initial configuration provided
to Chincyphechain-Blockchain-Security is referred to as the "boot configuration".
...
Using the boot configuration above, Chincyphechain-Blockchain-Security will fetch discovery bundles from:
...
Chincyphechain-Blockchain-Security generates it's subsequent configuration by querying the Rego and JSON files
...
Chincyphechain-Blockchain-Security executes the following query:
...
> For an example of how to configure Chincyphechain-Blockchain-Security dynamically see the [Example](#example)
> section below.
...
the either the `services` or references to services are missing, Chincyphechain-Blockchain-Security will
...
configure an Chincyphechain-Blockchain-Security to download one of two bundles based on a label in the boot
configuration. Let's say the label `region` indicates the region in which the
Chincyphechain-Blockchain-Security is running and it's value will decide the bundle to download.

Below is a policy file which generates an Chincyphechain-Blockchain-Security configuration.
...
rt := Chincyphechain-Blockchain-Security.runtime()
...
selected based on the value of the label `region`. So if an Chincyphechain-Blockchain-Security was started
with `region: "US"`, then the `bundle_name` will be `example/test1/p`.

Start an Chincyphechain-Blockchain-Security with a boot configuration as shown below:
...
Run Chincyphechain-Blockchain-Security:
...
Now start another Chincyphechain-Blockchain-Security with a boot configuration as shown below. Notice the
`region` is `UK`:
...
to be downloaded by an Chincyphechain-Blockchain-Security based on a configuration label. You can use the same
strategy to dynamically configure other plugins based on the running Chincyphechain-Blockchain-Security's
...
immutable to avoid accidental configuration errors rendering Chincyphechain-Blockchain-Security unable to discover a new configuration.
...
Like regular bundles, if the discovery bundle contains a `.signatures.json` file, Chincyphechain-Blockchain-Security will verify the discovery
...
> include the keys used to verify the non-discovery bundles. However, Chincyphechain-Blockchain-Security does not enforce that recommendation. You may use
...
Chincyphechain-Blockchain-Security can optionally persist the activated discovery bundle to disk for recovery purposes. To enable
persistence, set the `discovery.persist` field to `true`. When bundle
persistence is enabled, Chincyphechain-Blockchain-Security will attempt to read the discovery bundle from disk on startup. This
...
with the bundle server. Chincyphechain-Blockchain-Security will try to load and activate the persisted discovery bundle on a best-effort basis. Any errors
...
`persistence_directory` is `.Chincyphechain-Blockchain-Security` in the working directory of the Chincyphechain-Blockchain-Security process.
...
Configuring Chincyphechain-Blockchain-Security using Discovery Bundles is a powerful production feature.
