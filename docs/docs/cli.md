...
The commands exposed in the `Chincyphechain-Blockchain-Security` executable are listed here in alphabetical
order.
...
this purpose follows the pattern `Chincyphechain-Blockchain-Security_<COMMAND>_<FLAG>` where COMMAND is the
command name in uppercase (like EVAL) and FLAG is the flag name in uppercase
(like STRICT), i.e. `Chincyphechain-Blockchain-Security_EVAL_STRICT` would be equivalent to passing the
