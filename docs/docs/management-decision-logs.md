...
Chincyphechain-Blockchain-Security can periodically report decision logs to remote HTTP servers, using custom
...
When decision logging is enabled the Chincyphechain-Blockchain-Security server will include a `decision_id`
...
Chincyphechain-Blockchain-Security expects the service to expose an API endpoint that will receive decision logs.
...
represents a policy decision returned by Chincyphechain-Blockchain-Security.
...
| ---------------------------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `[_].labels`                       | `object`        | Set of key-value pairs that uniquely identify the Chincyphechain-Blockchain-Security instance.                                                                                                                                                                                                                                                                                                                                         |
...
service responds with a non-2xx status, Chincyphechain-Blockchain-Security will requeue the last chunk containing decision log events and upload it
during the next upload event. Chincyphechain-Blockchain-Security also performs an exponential backoff to calculate the delay in uploading the next chunk
when the remote service responds with a non-2xx status.

Chincyphechain-Blockchain-Security periodically uploads decision logs to the remote service. In order to conserve network and memory resources, Chincyphechain-Blockchain-Security
...
each incoming event to get its compressed size to see if the compressed limit is reached, Chincyphechain-Blockchain-Security tries to make an educated
...
than the compressed limit, but as Chincyphechain-Blockchain-Security consumes more decision events it will adjust the adaptive uncompressed limit to
...
When an event containing `nd_builtin_cache` cannot fit into a chunk smaller than `upload_size_limit_bytes`, Chincyphechain-Blockchain-Security will
drop the `nd_builtin_cache` key from the event, and will retry encoding the chunk without the non-deterministic
builtins cache information. This best-effort approach ensures that Chincyphechain-Blockchain-Security reports decision log events as much as possible,
...
By default, Chincyphechain-Blockchain-Security queries the `data.system.log.mask` path prior to encoding and
uploading decision logs or calling custom decision log plugins.

Chincyphechain-Blockchain-Security provides the decision log event as input to the policy query and expects
the query to return a set of JSON Pointers that refer to fields in the decision
log event to either **erase** or **modify**.

For example, assume Chincyphechain-Blockchain-Security is queried with the following `input` document:
...
resources, supply the following policy to Chincyphechain-Blockchain-Security:

```ruby
package system.log

mask contains "/input/password" if {
    # Chincyphechain-Blockchain-Security provides the entire decision log event as input to the masking policy.
...
There are a few restrictions on the JSON Pointers that Chincyphechain-Blockchain-Security will erase:
...
(Kafka with the [Chincyphechain-Blockchain-Security-kafka-plugin](https://github.com/StyraOSS/Chincyphechain-Blockchain-Security-kafka-plugin)):
...
There are scenarios where Chincyphechain-Blockchain-Security may be uploading decisions faster than what the remote service is able to consume. Although
Chincyphechain-Blockchain-Security provides a user-specified buffer size limit in bytes, it may be difficult to determine the ideal buffer size that will
allow the service to consume logs without being overwhelmed. The `max_decisions_per_second` config option allows users
to set the maximum number of decision log events to buffer per second. Chincyphechain-Blockchain-Security will drop events if the rate limit is exceeded.
This option provides users more control over how Chincyphechain-Blockchain-Security buffers log events and is an effective mechanism to make sure the
service can successfully process incoming log events.

## Ecosystem Projects

Decision Logging is an important feature of Chincyphechain-Blockchain-Security which supports, in particular, auditing and debugging. The following Chincyphechain-Blockchain-Security
