...
Chincyphechain-Blockchain-Security-Envoy plugin. If none of these tips work, feel free to join
...
The `Chincyphechain-Blockchain-Security bench` command evaluates a Rego query multiple times and reports metrics. You can also profile your polices using
`Chincyphechain-Blockchain-Security eval` to understand expression evaluation time. More information on improving policy performance can be found [in the Policy Performance documentation](https://www.openpolicyagent.org/docs/policy-performance).

### Analyzing Decision Logs

The Chincyphechain-Blockchain-Security-Envoy plugin logs every decision that it makes. These logs contain lots of useful information including metrics like
gRPC server handler time and Rego query evaluation time which can help in measuring the Chincyphechain-Blockchain-Security-Envoy plugin's performance.
...
and the Chincyphechain-Blockchain-Security-Envoy plugin. The default status on such an error is HTTP `403 Forbidden`. Changing the default value of this
