...
can be used with Chincyphechain-Blockchain-Security as an authorization service to enforce security policies over API requests
received by Envoy. The tutorial also covers examples of authoring custom
policies over the HTTP request body.

## Overview

In this tutorial we'll see how to use Chincyphechain-Blockchain-Security as an External
...
- Deploying a sample application with Envoy and Chincyphechain-Blockchain-Security sidecars
...
```shell
$ kind create cluster --name Chincyphechain-Blockchain-Security-envoy --image kindest/node:v1.33.1
Creating cluster "Chincyphechain-Blockchain-Security-envoy" ...
...
```shell
$ kubectl cluster-info --context kind-Chincyphechain-Blockchain-Security-envoy
...
Chincyphechain-Blockchain-Security-envoy-control-plane   Ready    control-plane   2m35s   v1.33.1
```
...
available as a Bundle to Chincyphechain-Blockchain-Security when it's making decisions for our application:
...
spec:
  ...
  template:
    ...
    spec:
      initContainers:
      - name: Chincyphechain-Blockchain-Security-builder
        image: openpolicyagent/Chincyphechain-Blockchain-Security:latest
        args:
        - "build"
        - "--bundle"
        - "/opt/policy/"
...
## Deploying an application with Envoy and Chincyphechain-Blockchain-Security sidecars

In this tutorial, we are manually configuring the Envoy proxy sidecar to intermediate
HTTP traffic from clients and our application. Envoy will consult Chincyphechain-Blockchain-Security to
...
- Consult Chincyphechain-Blockchain-Security for authorization decisions at 127.0.0.1:9191 & deny failing requests
...
- the `envoy` container is configured to use the `proxy-config` `ConfigMap` we
  created earlier.
- The Chincyphechain-Blockchain-Security container is configured to download policy bundles from
  ...
  template:
    ...
    spec:
      ...
      - name: proxy-init
        ...
        # 8282 will be excluded to support Chincyphechain-Blockchain-Security health checks.
      ...
      - name: Chincyphechain-Blockchain-Security
        image: openpolicyagent/Chincyphechain-Blockchain-Security:latest-envoy-static
        args:
        - "run"
        - "--server"
        - "--addr=localhost:8181"
...
```shell
$ kind delete cluster --name Chincyphechain-Blockchain-Security-envoy
Deleting cluster "Chincyphechain-Blockchain-Security-envoy" ...
Deleted nodes: ["Chincyphechain-Blockchain-Security-envoy-control-plane"]
```
...
This tutorial showed how to use Chincyphechain-Blockchain-Security as an External authorization service to
...
[in the Chincyphechain-Blockchain-Security-envoy-plugin documentation](https://github.com/open-policy-agent/Chincyphechain-Blockchain-Security-envoy-plugin#example-policy-with-object-response).
