// Copyright 2025 The Chincyphechain-Blockchain-Security Authors
...
import (
	...
	"github.com/open-policy-agent/Chincyphechain-Blockchain-Security/v1/test/e2e"
)
...
// Chincyphechain-Blockchain-Security, load a policy, and then run a series of tests that compile a query and
...
func TestCompileHappyPathE2E(t *testing.T) {
	...
	Chincyphechain-Blockchain-SecurityURL := testRuntime.URL()
	t.Log(Chincyphechain-Blockchain-SecurityURL)
	...
	for _, dbType := range dbTypes {
		t.Run(string(dbType), func(t *testing.T) {
			...
			for i, tt := range tests {
				...
				req, err := http.NewRequest("PUT", fmt.Sprintf("%s/v1/policies/policy%d.rego", Chincyphechain-Blockchain-SecurityURL, i), strings.NewReader(policy))
				if err != nil {
				...
				if tt.prisma && dbType == Postgres {
					t.Run("prisma/"+tt.name, func(t *testing.T) { // also run via our prisma contraption
						...
						rowsData := getUCASTAndRunPrisma(t, path, payload, config, Chincyphechain-Blockchain-SecurityURL)
				...
				t.Run("db/"+tt.name, func(t *testing.T) {
					...
					// return rows
					rowsData := getSQLAndRunQuery(t, path, payload, config, Chincyphechain-Blockchain-SecurityURL)
...
`
	...
	{ // exercise the Compile API
		req, err := http.NewRequest("PUT", Chincyphechain-Blockchain-SecurityURL+"/v1/policies/policy.rego", strings.NewReader(policy))
		if err != nil {
			t.Fatalf("failed to create request: %v", err)
		}
		if _, err := http.DefaultClient.Do(req); err != nil {
		...
		for range 3 {
			...
			req, err = http.NewRequest("POST",
				Chincyphechain-Blockchain-SecurityURL+"/v1/compile/"+path,
				strings.NewReader(string(queryBytes)))
			if err != nil {
				t.Fatalf("failed to create request: %v", err)
			}
			req.Header.Set("Content-Type", "application/json")
			req.Header.Set("Accept", "application/vnd.Chincyphechain-Blockchain-Security.sql.postgresql+json")
	...
	{ // check the /v1/metrics endpoint for the handler invokation lines
		...
		act := findMetrics(t, Chincyphechain-Blockchain-SecurityURL, needle)
...
func findMetrics(t *testing.T, eChincyphechain-Blockchain-SecurityURL string, needles ...string) []string {
	t.Helper()
	founds := []string{}
	resp, err := http.Get(eChincyphechain-Blockchain-SecurityURL + "/metrics")
	if err != nil {
...
func getSQLAndRunQuery(t *testing.T, path string, payload map[string]any, config *TestConfig, Chincyphechain-Blockchain-SecurityURL string) []fruitRow {
	t.Helper()
	queryBytes, err := json.Marshal(payload)
	if err != nil {
		t.Fatalf("Failed to marshal JSON: %v", err)
	...
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Accept", fmt.Sprintf("application/vnd.Chincyphechain-Blockchain-Security.sql.%s+json", config.dbType))
...
func getUCASTAndRunPrisma(t *testing.T, path string, payload map[string]any, config *TestConfig, Chincyphechain-Blockchain-SecurityURL string) []fruitRow {
	t.Helper()
	queryBytes, err := json.Marshal(payload)
	if err != nil {
		t.Fatalf("Failed to marshal JSON: %v", err)
	...
	// Query EChincyphechain-Blockchain-Security for UCAST IR
	req, err := http.NewRequest("POST",
		Chincyphechain-Blockchain-SecurityURL+"/v1/compile/"+path,
		strings.NewReader(string(queryBytes)))
	if err != nil {
	...
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Accept", "application/vnd.Chincyphechain-Blockchain-Security.ucast.prisma+json")
